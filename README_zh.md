...
![sshx 系统架构](docs/architecture.svg)
...
- 兼容 runtime 位于 `~/.sshx_server/targets/<TargetID>/runtimes/<RuntimeID>`，可服务多个应用 Context 和 session。
...
1. **连接**：`sshx remote` 打开正常 SSH 会话，并在 `~/.sshx_server/targets/<TargetID>/runtimes/<RuntimeID>` 下启动兼容 runtime。
