...
每个用户可见会话仍是普通的 OpenSSH 连接；容器目标则使用 `docker exec`。隐藏的控制通道和可选 RemoteFS 通道将客户端连接到目标端的共享服务器，本地共享守护进程则统一管理所有终端使用的 DNS 记录和 TCP 转发。自动端口转发使用 SSH `direct-tcpip`；Docker 目标支持 shell 和命令桥，但不启用自动端口扫描。
